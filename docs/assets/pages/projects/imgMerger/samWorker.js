...
async function doInit() {
  try {
    ...
    model     = await lib.SamModel.from_pretrained(SAM_MODEL_ID,     { progress_callback: prog });
...
async function doEncode({ imgIdx, pixels, width, height }) {
  try {
    ...
    // Serialize embeddings to plain typed arrays so the main thread can cache
    // them in normal JS heap. Each tensor becomes { type, dims, data }.
    const serialized = {};
    const transfers  = [];
    for (const [key, tensor] of Object.entries(embeddings)) {
      const buf = tensor.data.buffer.slice(0);  // copy — worker's WASM heap stays valid
...
async function doDecode({ imgIdx, x, y, decodeSize, embeddings, originalSizes, reshapedSizes }) {
  try {
    ...
    // Cap the mask output size for faster post-processing.
