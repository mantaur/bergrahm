...
async function computeVoronoiMap(images, placements, W, H) {
  ...
  const activePs = placements.filter((p) => {
    const e = images[p.imgIdx];
    return e.polygons && e.polygons.length > 0;
  });
  if (activePs.length === 0) return ownerMap;
  postProgress(20);

  await parallelChamfer(
    activePs.length,
    W,
    H,
    (j) => {
      const p = activePs[j],
        entry = images[p.imgIdx];
...
async function computeDitherMap(images, placements, W, H, seed, exp) {
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      if (k0 < 0) continue;
      const k1 = top1K[i];
      if (k1 < 0 || top0Dist[i] === 0) {
        ownerMap[i] = placements[k0].imgIdx;
        continue;
      }
