...
async function doInit() {
  try {
    ...
    while (true) {
      ...
      if (total > 0) self.postMessage({ type: "progress", text: "YOLO model " + Math.round((received / total) * 100) + "%" });
    }
...
function nms(candidates) {
  const byClass = {};
  for (const d of candidates) (byClass[d.classId] = byClass[d.classId] || []).push(d);
  ...
  for (const cls of Object.values(byClass)) {
    ...
    for (let i = 0; i < cls.length; i++) {
      ...
      keep.push(cls[i]);
      for (let j = i + 1; j < cls.length; j++) if (!suppressed[j] && boxIou(cls[i].box640, cls[j].box640) > IOU_THRESH) suppressed[j] = 1;
    }
  }
...
async function doEncode({ imgIdx, pixels, width, height }) {
  try {
    ...
    for (let i = 0; i < 8400; i++) {
      ...
      const cx = det[0 * 8400 + i],
        cy = det[1 * 8400 + i];
      const bw = det[2 * 8400 + i],
        bh = det[3 * 8400 + i];
      const box640 = [Math.max(0, cx - bw / 2), Math.max(0, cy - bh / 2), Math.min(640, cx + bw / 2), Math.min(640, cy + bh / 2)];
    ...
    for (const d of dets) {
      ...
      const bbox = [Math.max(0, Math.round((bx1 - padX) / scale)), Math.max(0, Math.round((by1 - padY) / scale)), Math.min(width, Math.round((bx2 - padX) / scale)), Math.min(height, Math.round((by2 - padY) / scale))];

      segments.push({ classId: d.classId, className: COCO_NAMES[d.classId], score: d.score, bbox, mask, maskW: outW, maskH: outH });
